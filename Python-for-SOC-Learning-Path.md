...
## 🥉 Deprioritized — Not the Focus Right Now

Not dismissing these — they're legitimate, well-taught parts of the course — but they don't serve the SOC/automation goal directly, so I'm consciously setting them aside rather than letting the course's breadth dilute my focus.

- **Python Game Development**, **Turtle** — fun, not relevant here
- **Python GUI Desktop App Development**, **Tkinter** — only relevant if I later want to wrap a tool in a GUI; command-line/scripted tools are the actual SOC standard
- **Front-End Web Development** — HTML5, CSS3, Bootstrap 4, WTForms, general Web Design — only relevant if the Flask work above grows into something needing a real interface
- **Selenium WebDriver** — has real security use cases (automated web app testing, some OSINT), but it's a deeper, narrower tool I'll pick up if a specific task actually calls for it
- **NumPy, Plotly** — supportive of Pandas/Matplotlib but not essential on their own for where I'm starting
- **Deployment (GitHub Pages, Heroku, Gunicorn)**, **Authentication** — relevant once I'm actually shipping a tool for others to use; premature right now
- **Google Colab** — nice-to-have for occasional notebook work, but Jupyter locally covers the same need without depending on a cloud environment
